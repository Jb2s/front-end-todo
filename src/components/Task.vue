<template>
  <div
    class="task-card flex bg-white border border-gray-300 rounded-lg shadow-md p-5 transition-shadow duration-200 hover:shadow-lg"
  >
    <div class="task-content flex flex-col flex-grow">
      <span
        class="inline-block w-full px-2 py-1 text-md text-gray mb-2"
        :class="{
          'bg-green-200 rounded-sm': props.item.completed,
          'bg-yellow-200 rounded-sm': !props.item.completed,
        }"
      >
        <i>{{ props.item.completed ? 'Complète' : 'Planifiée' }}</i>
      </span>
      <h5
        class="task-title text-lg text-gray-900 cursor-pointer"
        :class="{ 'line-through': props.item.completed }"
        @click="handleTaskClick"
      >
        <i>{{ props.item.title }}</i>
      </h5>
      <h6 class="task-title text-md text-gray-700 cursor-pointer" @click="handleTaskClick">
      ...
      <p class="task-subtasks-count text-sm text-gray-600">
        <i>{{
          props.item.subTasks && props.item.subTasks.length > 0
            ? props.item.subTasks.length + ' sous tâche(s)'
            : 'Aucune tâche'
        }}</i>
      </p>
      <div class="flex justify-between items-center">
        <div class="task-actions">
          <p class="task-subtasks-count text-xs text-indigo-600">
            <i v-if="props.item.isShared">Partagée</i>
          </p>
        </div>
          ...
          <div v-if="!isCommunityRoute">
            <div class="relative group">
              <button
                ...
                class="text-indigo-900 bg-indigo-300 hover:bg-indigo-400 font-medium rounded-sm text-sm px-5 py-2.5 text-center inline-flex items-center"
                type="button"
              >
                Actions
                <svg
              ...
              <div
                class="absolute z-10 bg-white divide-y divide-indigo-100 rounded-sm shadow w-40 hidden group-hover:block"
              >
                <ul class="py-2 text-sm text-gray-700">
                  <li>
                    <button
                      @click.stop="handleToggleTask"
                      class="w-full flex items-center px-4 py-2 hover:bg-indigo-100"
                    >
                  ...
                  <li>
                    <button
                      @click.stop="handleLaunchEditTask"
                      class="w-full flex items-center px-4 py-2 hover:bg-indigo-100"
                    >
        ...
        <div v-if="!iskTaskMine(props.item)">
          <div v-if="getSharedByUserName(props.item)">
            <p class="text-xs font-medium text-indigo-500">
              <i> Par : {{ props.item.user.username }}</i>
            </p>
          </div>
        </div>
      </div>
